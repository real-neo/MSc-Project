...
def draw_pdf_ccdf(dataset, x_label):
    fit = powerlaw.Fit(dataset, discrete=True)

    fig, ax = plt.subplots()

    figCCDF = fit.plot_pdf(ax, color='b', linewidth=2, label='PDF')
    fit.power_law.plot_pdf(color='b', linestyle='--', ax=figCCDF, label='power law fit PDF')
    fit.plot_ccdf(color='r', linewidth=2, ax=figCCDF, label='CCDF')
    fit.power_law.plot_ccdf(color='r', linestyle='--', ax=figCCDF, label='power law fit CCDF')
...
def draw_population(file):
    ...
    draw_pdf_ccdf(population.Population, 'Population')
